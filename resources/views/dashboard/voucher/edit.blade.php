    ...
    <div class="row">
        <div class="col-lg-6">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form action="/dashboard/voucher/{{ $voucher->voucher_id }}" method="post">
                        @method('put')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control @error('batas_waktu') is-invalid @enderror"
                                id="batas_waktu" name="batas_waktu" value="{{ old('batas_waktu', $voucher->batas_waktu) }}">
                            @error('batas_waktu')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('status') is-invalid @enderror" id="status"
                                name="status">
                                <option value="aktif" {{ old('status', $voucher->status) === 'aktif' ? 'selected' : '' }}>
                                    Aktif</option>
                                <option value="kedaluwarsa"
                                    {{ old('status', $voucher->status) === 'kedaluwarsa' ? 'selected' : '' }}>Tidak Aktif
                                </option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">
